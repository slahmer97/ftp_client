...
void cmd_handler(const char*__cmd){
    ...
    *(tmp-1) = 0;
    ...
    switch (cmd){
        ...
        case CIAO:
            if(_status_ == DISCONNECTED){
                fprintf(stdout,"[!] you do not have an active connection.\n");
                break;
            }
            send_ciao();
    ...
    already_logged_in:
        fprintf(stdout,"[-] you have already an opened connection, if you wish to open new, please close the old one [Ciao] \n");
}